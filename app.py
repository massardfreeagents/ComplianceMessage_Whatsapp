...
App Streamlit independente — layout mobile-first (single column).
...
st.set_page_config(
    ...
    page_icon="📱",
    layout="centered",          # ← centered = respeita telas pequenas
...
/* bloco central nunca ultrapassa 720px — confortável em tablet e desktop */
...
.hero-sub {
    color: #25d366; font-size: 0.72rem; font-weight: 600;
    letter-spacing: 0.13em; text-transform: uppercase; margin-top: 0.25rem;
}
...
/* checkboxes e toggles maiores para touch */
...
def fetch_contatos(filtro: str):
    url  = f"{_api_url()}/chat/findContacts/{_instance()}"
    resp = requests.post(url, headers=_headers(), json={"where": {}}, timeout=20, verify=False)
    resp.raise_for_status()
    data = resp.json()
    ...
    resp = requests.post(
        ...
        json={
            ...
            "messages": [{
                "role": "user",
                "content": (
                    "Corrija apenas os erros de ortografia, acentuacao e gramatica do texto abaixo. "
...
FILTRO_CONTATOS = "ABC1234teste"  # ← troque pelo nome real

# ─── HERO + status de conexão ────────────────────────────────────────────────
api_ok = bool(_api_url() and os.getenv("WHATSAPP_API_KEY"))
status_html = (
    '<span style="color:#25d366;font-weight:700">● API conectada</span>'
...
# Injeta texto corrigido no campo antes de renderizar
...
col_img, col_corr = st.columns([1, 1])
with col_img:
...
# Preview da mensagem logo abaixo
if mensagem.strip():
    st.markdown(f'<div class="bubble">{mensagem}</div>', unsafe_allow_html=True)
    if com_imagem:
        st.caption("📎 Imagem ForExperts será anexada")
...
usar_contatos = st.checkbox("👤 Contatos por nome", value=False, key="cb_contatos")
...
if usar_grupos:
    ...
    if not jids_fixos:
        ...
        if grupos_sel:
            st.markdown(
                f'<span style="color:#25d366;font-size:0.8rem;font-weight:600">'
                f'✓ {len(grupos_sel)} grupo(s) selecionado(s)</span>',
                unsafe_allow_html=True,
            )
...
if usar_contatos:
    ...
    st.markdown(
        ...
        f'Filtro ativo: <span style="color:#25d366;font-family:\'DM Mono\',monospace;font-weight:600">'
        f'"{FILTRO_CONTATOS}"</span></div>',
        unsafe_allow_html=True,
    )

    if st.button("🔍 Carregar contatos", key="btn_contatos"):
    ...
    if st.session_state.contatos_cache:
        ...
        if contatos_sel:
            st.markdown(
                f'<span style="color:#25d366;font-size:0.8rem;font-weight:600">'
                f'✓ {len(contatos_sel)} contato(s) selecionado(s)</span>',
                unsafe_allow_html=True,
            )
...
if st.button(
    ...
    <div class="result-grid">
        <div class="stat-ok">
            <div class="stat-num-ok">{enviados}</div>
            <div class="stat-lbl" style="color:#25d366">Enviados</div>
        </div>
        <div class="stat-fail">
            <div class="stat-num-fail">{falhas}</div>
            <div class="stat-lbl" style="color:#f87171">Falhas</div>
        </div>
    </div>
    """, unsafe_allow_html=True)
